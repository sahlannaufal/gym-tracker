...
- **Storage:** LocalStorage via service di `lib/storage.ts` (key `gym_tracker_workouts_v1`, ada versioning + validasi)
- **Komponen client** (yang akses localStorage) wajib `"use client"`.
- **Routing:** App Router — `/` dashboard, `/workout/new` form, `/history` histori, `/progress` grafik.
- **Data model & hook:** `lib/types.ts` (Workout) & `lib/useWorkouts.ts` (state + add/remove).
