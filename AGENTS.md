...
- **Storage:** LocalStorage via service di `lib/storage.ts` (key `gym_tracker_workouts_v1` untuk workout & `gym_tracker_routine_v1` untuk rutin harian, ada versioning + validasi)
- **Komponen client** (yang akses localStorage) wajib `"use client"`.
- **Routing:** App Router — bottom nav (`components/BottomNav.tsx`): `/` dashboard, `/today` "Latihan Hari Ini" (segmented Latihan | Rutin, editor rutin via `?view=rutin`), `/workout/new` form (FAB +, terima `?exercise=` prefill), `/progress` "Progres" (segmented Riwayat | Grafik). `/routine` redirect → `/today?view=rutin`, `/history` redirect → `/progress`.
