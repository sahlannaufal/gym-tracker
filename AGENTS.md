...
- **Framework:** Next.js 16 (App Router) + React 19 + TypeScript — `npx next dev`
...
- **Domain:** aplikasi di `gym.abadikan.com`; situs marketing (landing `/aplikasi-tracking-gym`, blog, terms) di `abadikan.com`. Routing per-hostname di `proxy.ts` (konvensi proxy Next 16): `abadikan.com/` di-rewrite ke landing, route aplikasi di-301 ke `gym.abadikan.com`, URL marketing lama di `gym.abadikan.com` di-301 ke `abadikan.com`. Konstanta domain + helper CTA (`buildAuthUrl`) di `lib/site.ts`. Atribusi marketing lintas domain lewat query string (dikonsumsi `saveMarketingAttributionFromUrl` di `components/AuthForm.tsx`). PWA (`SerwistProvider`) dimatikan di domain marketing.
